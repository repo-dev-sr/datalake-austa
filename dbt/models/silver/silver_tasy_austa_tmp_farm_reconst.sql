...
{#- Customizacao AUSTA - reconstituicao (786 registros) -#}
{{
  config(
    ...
    merge_update_columns=[
      'cd_material', 'cd_estabelecimento',
      'ds_reconstituicao', 'ds_observacao', 'ie_situacao',
      'qt_volume', 'qt_concentracao',
    ...
    post_hook=post_hook_delete_source_tombstones('bronze_tasy_austa_tmp_farm_reconst', 'nr_sequencia', '_is_deleted')
  )
}}

WITH base AS (
  SELECT * FROM {{ ref('bronze_tasy_austa_tmp_farm_reconst') }} b
  {% if is_incremental() %}
...
, shaped AS (
  SELECT
    ...
    , {{ standardize_text_initcap('d.ds_reconstituicao', "'indefinido'") }}  AS ds_reconstituicao
    , {{ standardize_text_initcap('d.ds_observacao', "'indefinido'") }}      AS ds_observacao
    , {{ standardize_enum('d.ie_situacao', "'i'") }}              AS ie_situacao

    , {{ normalize_decimal('d.qt_volume', 4, -1) }}               AS qt_volume
    , {{ normalize_decimal('d.qt_concentracao', 4, -1) }}         AS qt_concentracao
