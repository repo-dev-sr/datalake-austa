...
{{
  config(
    ...
    merge_update_columns=[
      'nr_seq_lote',
      'ie_tipo_historico', 'ds_historico', 'ds_titulo',
      'dt_historico',
    ...
    post_hook=post_hook_delete_source_tombstones('bronze_tasy_ap_lote_historico', 'nr_sequencia', '_is_deleted')
  )
}}

WITH base AS (
  SELECT * FROM {{ ref('bronze_tasy_ap_lote_historico') }} b
  {% if is_incremental() %}
...
, shaped AS (
  SELECT
    ...
    , {{ standardize_enum('d.ie_tipo_historico', "'i'") }}        AS ie_tipo_historico
    , {{ standardize_text_initcap('d.ds_historico', "'indefinido'") }}       AS ds_historico
    , {{ standardize_text_initcap('d.ds_titulo', "'indefinido'") }}          AS ds_titulo

    , {{ standardize_date('d.dh_historico') }}                    AS dt_historico
