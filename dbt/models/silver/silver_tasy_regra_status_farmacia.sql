...
{{
  config(
    ...
    merge_update_columns=[
      'cd_status', 'cd_estabelecimento',
      'ds_regra', 'ie_situacao', 'ie_origem', 'ie_acao',
    ...
    post_hook=post_hook_delete_source_tombstones('bronze_tasy_regra_status_farmacia', 'nr_sequencia', '_is_deleted')
  )
}}

WITH base AS (
  SELECT * FROM {{ ref('bronze_tasy_regra_status_farmacia') }} b
  {% if is_incremental() %}
...
, shaped AS (
  SELECT
    ...
    , {{ fill_null_string('d.cd_status', "'indefinido'") }}       AS cd_status
