...
{#- Customizacao AUSTA - capas/etiquetas (850 registros) -#}
{{
  config(
    ...
    merge_update_columns=[
      ...
      'ds_capa', 'ds_etiqueta', 'ds_observacao', 'ie_situacao',
    ...
    post_hook=post_hook_delete_source_tombstones('bronze_tasy_austa_tmp_farm_capa', 'nr_sequencia', '_is_deleted')
  )
}}

WITH base AS (
  SELECT * FROM {{ ref('bronze_tasy_austa_tmp_farm_capa') }} b
  {% if is_incremental() %}
...
, shaped AS (
  SELECT
    ...
    , {{ standardize_text_initcap('d.ds_capa', "'indefinido'") }}            AS ds_capa
    , {{ standardize_text_initcap('d.ds_etiqueta', "'indefinido'") }}        AS ds_etiqueta
