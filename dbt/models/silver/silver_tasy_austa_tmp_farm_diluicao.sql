...
{#- Customizacao AUSTA - diluicoes (1.307 registros) -#}
{{
  config(
    ...
    merge_update_columns=[
      ...
      'ds_diluicao', 'ds_observacao', 'ie_situacao',
    ...
    post_hook=post_hook_delete_source_tombstones('bronze_tasy_austa_tmp_farm_diluicao', 'nr_sequencia', '_is_deleted')
  )
}}

WITH base AS (
  SELECT * FROM {{ ref('bronze_tasy_austa_tmp_farm_diluicao') }} b
  {% if is_incremental() %}
...
, shaped AS (
  SELECT
    ...
    , {{ standardize_text_initcap('d.ds_diluicao', "'indefinido'") }}        AS ds_diluicao
