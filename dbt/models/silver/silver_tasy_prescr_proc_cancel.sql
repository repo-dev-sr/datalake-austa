...
{{
  config(
    ...
    merge_update_columns=[
      'nr_prescricao', 'nr_seq_proc', 'nr_seq_material', 'nr_seq_motivo',
      'ie_agrupador', 'ie_tipo_cancel', 'ds_motivo', 'ds_justificativa',
      'dt_cancelamento',
    ...
    post_hook=post_hook_delete_source_tombstones('bronze_tasy_prescr_proc_cancel', 'nr_sequencia', '_is_deleted')
  )
}}

WITH base AS (
  SELECT * FROM {{ ref('bronze_tasy_prescr_proc_cancel') }} b
  {% if is_incremental() %}
...
, shaped AS (
  SELECT
    ...
    , {{ fill_null_bigint('d.nr_seq_motivo', -1) }}               AS nr_seq_motivo

    , {{ standardize_enum('d.ie_agrupador', "'i'") }}             AS ie_agrupador
    , {{ standardize_enum('d.ie_tipo_cancel', "'i'") }}           AS ie_tipo_cancel
    , {{ standardize_text_initcap('d.ds_motivo', "'indefinido'") }}          AS ds_motivo
