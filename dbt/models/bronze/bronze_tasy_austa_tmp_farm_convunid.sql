{{ config(
    ...
    tags=['bronze','tasy','farmacia','customizacao_austa']
) }}

{% set raw_path = "s3a://austa-lakehouse-prod-data-lake-169446931765/raw/raw-tasy/stream/austa.TASY.AUSTA_TMP_FARM_CONVUNID/" %}
...
SELECT
    -- PK (customizacao AUSTA — colunas inferidas; validar com analista)
    r.nr_sequencia
  ...
  , r.cd_unidade_origem
  , r.cd_unidade_destino

    -- Atributos de conversao de unidade (AUSTA)
  , r.qt_conversao
  , r.ds_observacao
  , r.ie_situacao
