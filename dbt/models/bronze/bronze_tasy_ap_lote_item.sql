...
{% set raw_path = "s3a://austa-lakehouse-prod-data-lake-169446931765/raw/raw-tasy/stream/tasy.TASY.AP_LOTE_ITEM/" %}
...
SELECT
  ...
  , r.nr_seq_mat_hor
  , r.cd_material
  , r.cd_material_original
  , r.cd_unidade_medida
  , r.cd_local_consistido
  , r.nr_prescricao
  , r.nr_seq_prescr_mat
  ...
  , CAST(r.dt_geracao AS TIMESTAMP)           AS dh_geracao
  , CAST(r.dt_entrega AS TIMESTAMP)           AS dh_entrega
  , CAST(r.dt_recebimento AS TIMESTAMP)       AS dh_recebimento
